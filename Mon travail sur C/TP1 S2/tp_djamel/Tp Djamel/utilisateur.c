...
int Detect_sim(int a,int b,int i)///une fonction qui fait la comparaison entre deux nombres entrés
{
int k=0,cpt=0 , v ;///et fait retournner "10" si les chiffres communs sont égales à i.
int s,t;
do {
s=a % 10;
t=b % 10;
if ( s==t){
        k=k+1 ;///à chaque fois qu'il y a un chiffre commun on incrémente le k.
}
 a=a/10;
 b=b/10;
 cpt=cpt+1;
...
void suggestion_par_octet ()
{
    ...
    ///on trouve le maillon contenant le nom entré.
...
///on cherche (en bas) les utilisateurs qui ont des octets simmilaires .
    utilisateur *p;
    int cpt=0,trouv=0; int i ; int s=8;///s commence avec 8 car les octets qui sont identiques sont plus propriétaire
    while((s>4)&&(cpt<5)){///des autres et aprés avoir parcourus toute la liste on décrémente s jusqu'a ce qu'on arrive à s=5
...
if (cpt==0)///s'il le nombre de suggestion est nulle on écrit "aucune suggestion trouvée".
    {
        printf("\n\n");
...
char name[50];
scanf("%s",name);///on cherche le maillon contenant le nom entré.
utilisateur* tete = detect_lettre(name); utilisateur* user ;
    ...
    while (tete != NULL && trouve == 0 )
    {
        if (strcmp(tete->nom,name)== 0 )
            {
                user = tete ;///user contient le maillon recherché.
                trouve = 1 ;
            }
        tete = suivant(tete);
    }
    ...
    while (user->followings[i]!=NULL){///on parcourt la liste des followers des followers de user (la répétition n'est pas une faute
        trouve=i;///pour sauvegarder la valeur de i car i va varier aprés.
    tete=user->followings[i];
        while(tete->followings[j]!=NULL)
            {
                i++;
                while (user->followings[i]!=NULL){
                if (strcmp(tete->followings[j],name)!=0){///pour ne pas suggérer le nom lui méme.
                    if (strcmp(tete->followings[j],(user->followings[i])->followings[k])==0){
                        sugg++;///on trouvons un follower commun on incrémente sugg.
                ...
                k=0;
                 if (sugg!=0){table[s]=mem*100+j*10+sugg;///on enregistre la valeur de sugg dans un tableau .avec le i et le j.
                  s++;}///et aprés on aura la fonction tableau_followers qui va trouver les 5 plus grandes valeurs de sugg
                 ///et va retourner le nom comme suggestion (comme on a aussi le i et le j).

                 sugg=0;///on réinitialise sugg.
                 mem=0;
                 j++;
...
j=0;
 ...
 tableau_followers( &table, s, nombre_followers(user), user)   ;///s représente la taille du tableau.
